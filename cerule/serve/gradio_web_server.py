...
from cerule.conversation import (default_conversation, conv_templates, SeparatorStyle)
from cerule.constants import LOGDIR
from cerule.util.utils import (build_logger, server_error_msg, violates_moderation, moderation_msg)

logger = build_logger("gradio_web_server", "gradio_web_server.log")

headers = {"User-Agent": "cerule Client"}
...
learn_more_markdown = ("""
### License
non-commercial use only, Please contact us on X/Linkedin @tensoic if you find any potential violation.
""")
...
def build_demo(embed_mode):
    ...
    with gr.Blocks(title="cerule", theme=gr.themes.Default(), css=block_css) as demo:
        state = gr.State()
        ...
        with gr.Row():
            ...
            with gr.Column(scale=8):
                chatbot = gr.Chatbot(elem_id="chatbot", label="cerule Bot", height=550)
                with gr.Row():
                    with gr.Column(scale=8):
                        textbox.render()
